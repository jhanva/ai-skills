...
name: smoke-test
description: >
  Smoke test rapido antes de merge o release de juego 2D en Godot 4. Lista cambios
  recientes (git diff), ejecuta checklist automatico (JSON valido, no hardcoded values,
  no TODOs sin owner, tests pasan si hay runner), checklist manual (juego arranca,
  player se mueve, feature funciona, no regresiones). Veredicto: PASS/FAIL con blockers.
argument-hint: "[pre-merge | pre-release]"
