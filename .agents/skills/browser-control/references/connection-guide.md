...
**Cookies no se transfieren:** copiar el directorio default a otro path no preserva las cookies porque estan cifradas con una key vinculada a la ruta original.
...
- Verificar que Chrome esta corriendo: `ps aux | grep -i chrome` en macOS/Linux o `tasklist | findstr chrome` en Windows
...
- Verificar que no hay otro proceso usando el puerto 9222: `lsof -i :9222` en macOS/Linux
...
- Puede ser un browser no-Chromium. Firefox y Safari no soportan CDP.
...
- Verificar manualmente: `find ~/Library -name DevToolsActivePort 2>/dev/null` en macOS.
