...
description: >
  ...
  visualizacion HTML interactiva. Usala cuando el usuario dice "codegraph",
  "knowledge graph", "mapea el proyecto", "grafo del codigo", o hace
  preguntas de arquitectura sobre un proyecto que ya tiene codegraph-out/.
  Si codegraph-out/graph.json ya existe, las preguntas sobre el codebase se
  responden consultando el grafo.
---
...
- Invocar explicitamente con `$codegraph`.
- Usar esta version nativa en `.agents/skills/codegraph/` sin modificar la capa `.claude/`.
- Resolver el directorio de la skill como `.agents/skills/codegraph` desde la raiz del repo.

Script principal: `.agents/skills/codegraph/scripts/codegraph.py`
...
```bash
python .agents/skills/codegraph/scripts/codegraph.py build RUTA
```
...
```bash
# Crear con apply_patch el archivo codegraph-out/labels.tmp.json:
# {"0": "Nombre comunidad 0", "3": "Nombre comunidad 3", ...}
python .agents/skills/codegraph/scripts/codegraph.py label RUTA --file codegraph-out/labels.tmp.json
```
...
```bash
python .agents/skills/codegraph/scripts/codegraph.py query RUTA "pregunta" [--dfs] [--budget 2000]
python .agents/skills/codegraph/scripts/codegraph.py path RUTA "ConceptoA" "ConceptoB"
python .agents/skills/codegraph/scripts/codegraph.py explain RUTA "Concepto"
```
...
profundo de docs, usar tareas o subagentes de Codex con el prompt de
`references/semantic-extraction.md` y fusionar con `merge-fragments`. Si el
entorno de Codex no permite despachar subagentes, hacer la extraccion
semantica inline siguiendo el mismo prompt. No hacerlo por defecto.
