...
- Esta skill está pensada para invocación explícita con `$ml-ondevice`.
...
```
# Buscar ML existente
Busca con `rg -n`: "MediaPipe|MLKit|TFLite|tensorflow|onnxruntime|ImageEmbedder|ImageClassifier|ObjectDetector"

# Buscar wrappers existentes
Busca con `rg -n`: "Embedder|Classifier|Detector|Segmenter" en archivos .kt
```
