...
2. Leer los pesos ajustados del dominio desde `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/domains.md`
...
2. Si el usuario pidió una técnica específica (anime, voxel, tilt-shift, etc.), leer `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/techniques.md`
3. Aplicar los pesos del dominio (más énfasis donde el peso es mayor)
4. Verificar contra las reglas de hierro
5. Para plataformas específicas (Midjourney, SD), leer `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/platforms.md` para adaptar sintaxis
...
| Archivo | Cuándo leer |
|---|---|
| `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/domains.md` | SIEMPRE — contiene los pesos por dominio |
| `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/techniques.md` | Cuando el usuario pide un estilo/técnica específica |
| `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/platforms.md` | Cuando el prompt NO es para Gemini (Midjourney, SD, DALL-E) |
| `${CLAUDE_PLUGIN_ROOT}/references/prompt-artist/text-safety.md` | Cuando hay texto en la imagen o el prompt puede triggear safety filters |
